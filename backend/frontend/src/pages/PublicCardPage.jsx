...
export default function PublicCardPage() {
  ...
  return (
    <div className="w-full max-w-[680px] mx-auto pb-16 px-4 sm:px-6">
      ...
      {!loading && !error && (
        <>
          {/* Cover Image */}
          <div className="relative rounded-b-3xl overflow-hidden shadow-sm bg-gray-200 dark:bg-gray-800 aspect-[8/3] sm:aspect-[16/6]">
            {profile.cover ? <img loading="lazy" src={profile.cover} alt="cover" className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gradient-to-br from-sky-400 to-indigo-600" />}
          </div>

          {/* Avatar overlapping */}
          <div className="relative -mt-16 flex items-end justify-center px-6">
            <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-full ring-4 ring-white dark:ring-gray-900 overflow-hidden shadow-lg bg-gray-100 dark:bg-gray-700">
          ...
          <div className="px-6 mt-4 space-y-3">
            <div>
              ...
              {(profile.title || profile.company) && <p className="text-sm text-gray-600 dark:text-gray-400 leading-snug">{[profile.title, profile.company].filter(Boolean).join(' · ')}</p>}
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-2">
              {actions.map((a) => (
                <a key={a.key} href={a.href} className={`w-full sm:flex-1 h-11 inline-flex items-center justify-center rounded-full text-sm font-medium transition shadow-sm ${a.primary ? 'bg-[#031a2c] text-white hover:bg-[#05243b]' : 'bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-100 hover:bg-gray-300/70 dark:hover:bg-gray-700'}`}>
                  {a.label}
                </a>
              ))}
            </div>
